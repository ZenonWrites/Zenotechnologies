...
const AwsIcon = ({
  ...
  // This icon's path data is drawn on a ~244×153 canvas (wordmark + swoosh),
  // not the template's default 24×24 square — sized to match so nothing clips.
  const baseWidth = 244;
  const baseHeight = 153;
  const viewBoxOffset = -padding;
  const viewBox = `${viewBoxOffset} ${viewBoxOffset} ${baseWidth + padding * 2} ${baseHeight + padding * 2}`;
